...
4. Verify Source strikes accepted deletions in place, marks working insertions once, and keeps pending text editable. Verify adjacent substitutions coalesce into readable replacement hunks and re-editing a pending insertion retains its hunk identity. Supported Visual blocks must show one accepted and one proposed rendering; cross-block changes must remain accurate in Source and Review without an incorrect Visual annotation. Verify Review shows one compact card per hunk and every server-stamped contributor. Selecting inline markup must focus its card. Have a maintainer add text in **Suggesting**, then accept the whole proposal; repeat and reject it. Confirm all sessions converge and checkpoint history retains each decision.
