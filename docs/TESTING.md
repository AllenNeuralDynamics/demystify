...
2. Type in Source and verify the local preview updates while the status says **Saving suggestion...**. After the short pause, verify every Suggestion, Viewer, and maintainer session sees one attributed proposal without a submit button. Accepted `content` must remain unchanged.
3. Open a Visual paragraph and type without finishing the editor. Verify other sessions remain unchanged until **Save visual edit**, then receive one attributed proposal through the same review model.
4. Verify Source strikes accepted deletions in place and marks each proposed insertion once. Supported Visual blocks must show accepted text and each proposed alternative; cross-block changes must remain accurate in Source and Review without an incorrect Visual annotation. Mouse, Enter, and Space activation of inline markup must focus its thread. Accept one suggestion and reject another; only the selected suggestion may change canonical source, and both decisions must move into the Resolved inbox.
5. Have two reviewers suggest different ranges concurrently, then the same range as alternatives. Verify each proposal retains its own author and decision controls rather than merging contributors into one checkpoint. Editing a projected proposal should create a newer superseding record without deleting history.
...
7. Verify maintainers continue to coedit accepted source live in **Editing** and may save accepted MyST while unrelated suggestions remain open. Accepting one proposal must not decide another.
8. Connect a GitHub account lacking repository write permission. Verify the room remains in Suggestion mode but new presence, comments, replies, and suggestions use the `github:<id>` actor. Reconnect anonymously and verify the share-session actor remains stable.
...
10. Run `npm run test:collaboration:load`. Require 5 editors and 95 viewers to synchronize a 100,000-character manuscript, awareness, and one update from each editor within the printed reliability thresholds.
