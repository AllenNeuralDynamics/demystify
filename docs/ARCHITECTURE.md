...
Sharing is capability-based and has three roles. A **maintainer** is authenticated through GitHub and has repository write access. A **guest editor** holds a room-scoped 256-bit capability and may submit Yjs manuscript, bibliography, metadata, and comment updates without repository access. A **viewer** holds an independent capability and may receive awareness and initial synchronization but cannot submit Yjs updates. Link secrets live only in URL fragments, are exchanged for versioned HTTP-only room grants, and are stored only as SHA-256 hashes with creation and expiration times. Repository binding, snapshots, pull requests, revisions, sharing administration, and GitHub comment APIs require maintainer access. Rotating, revoking, or expiring one capability closes only sockets using that role.
